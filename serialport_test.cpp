...
void read_port(int fd)
{
	...
	while(sign != 'q')
	{
		printf("waiting for read port...\n");
		if(read(fd, &sign, 1) > 0)
			printf("get data from port : %c\n", sign);
	}
}
...
int main(int argc, char *argv[])
{
	if(argc != 3)
	{
		printf("usage : exec read_port write_port\n");
	}
	int fd_r = open_port(argv[1]);
	int fd_w = open_port(argv[2]);

	config_port(fd_r);
	config_port(fd_w);

	std::thread t(read_port, fd_r);
